...
const TOUR_STEPS = [
  ...
  },
  {
    ...
    body:  'Your baseline business diagnosis across 6 dimensions. Takes 10 minutes. No tools needed. Retake it after major business changes — a new hire, a pivot, a fundraise.',
  },
  {
    id:    'tour-ai-card',
    title: 'AI Readiness',
  ...
  },
  {
    ...
    body:  "Select a dimension to explore your signals, or take your assessment to generate your first diagnosis. Your business OS is ready — let's go.",
  },
]
...
export function DashboardTour({ guideDismissed }: { guideDismissed: boolean }) {
  ...
  const measure = useCallback(() => {
    ...
    if (!el) {
      // Final step (tour-done) — no element, show centered card
      setRect({
      ...
      {showWelcomeCard && (
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ fontSize: 10, fontWeight: 700, color: '#3B82F6', letterSpacing: '0.15em', textTransform: 'uppercase', display: 'block', marginBottom: 8 }}>
            ...
            <p style={{ fontSize: 14, color: '#94A3B8', margin: '0 0 28px', lineHeight: 1.6 }}>
              Your business OS is ready. Take a 2-minute tour to understand what each section does — so you know exactly where to go when you need answers.
            </p>
      ...
      {createPortal(
        <>
          {active && (
            <>
              ...
              {rect && (
                <div
                  key={`card-${step}`}
                  style={{ position: 'fixed', top: cardTop, left: cardLeft, width: CARD_W, zIndex: 10002, background: '#0F172A', borderRadius: 14, border: '1px solid #334155', padding: 20, boxShadow: '0 20px 60px rgba(0,0,0,0.8)', fontFamily: 'Inter, -apple-system, sans-serif', animation: 'tourIn 0.22s ease' }}
                >
          ...
          <div style={{ position: 'fixed', bottom: 28, right: 28, zIndex: 9999 }}>
            {helpOpen && (
              <div style={{ position: 'absolute', bottom: 56, right: 0, background: '#0F172A', borderRadius: 12, border: '1px solid #1E293B', padding: 18, width: 220, boxShadow: '0 8px 32px rgba(0,0,0,0.4)', fontFamily: 'Inter, sans-serif' }}>
                ...
                <button onClick={doRestart} style={{ width: '100%', padding: 9, background: '#2563EB', color: '#fff', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Restart tour →</button>
              </div>
            )}
            <button
