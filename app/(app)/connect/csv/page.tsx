...
function downloadTemplate(template: { id: string; example: string[][]; color?: string; bg?: string }) {
...
function CSVUploadContent() {
  ...
  const TEMPLATES = [
    {
      id: 'support',
      name: t('connect.csv_tmpl_support'),
      description: t('connect.csv_tmpl_support_desc'),
    ...
    {
      id: 'orders',
      name: t('connect.csv_tmpl_orders'),
      description: t('connect.csv_tmpl_orders_desc'),
    ...
    {
      id: 'satisfaction',
      name: t('connect.csv_tmpl_satisfaction'),
      description: t('connect.csv_tmpl_satisfaction_desc'),
    ...
    {
      id: 'velocity',
      name: t('connect.csv_tmpl_velocity'),
      description: t('connect.csv_tmpl_velocity_desc'),
    ...
    {
      id: 'financial',
      name: t('connect.csv_tmpl_financial'),
      description: t('connect.csv_tmpl_financial_desc'),
      icon: '💵', color: '#059669', bg: '#ECFDF5',
    ...
    {
      id: 'marketing',
      name: t('connect.csv_tmpl_marketing'),
      description: t('connect.csv_tmpl_marketing_desc'),
      icon: '📣', color: '#D97706', bg: '#FFFBEB',
  ...
  return (
    <main dir={isAr ? 'rtl' : 'ltr'} style={{ minHeight: '100vh', background: '#F9FAFB', fontFamily: 'Inter, sans-serif', padding: 24 }}>
      <div style={{ maxWidth: 720, margin: '0 auto', paddingTop: 40 }}>
        ...
        <h1 style={{ fontSize: 26, fontWeight: 800, color: '#111827', marginBottom: 8 }}>{t('connect.csv_upload_title')}</h1>
        <p style={{ color: '#6B7280', fontSize: 15, marginBottom: 32, lineHeight: 1.6 }}>
          {t('connect.csv_upload_sub')}
        </p>

        <div style={{ marginBottom: 32 }}>
          <h2 style={{ fontSize: 16, fontWeight: 700, color: '#111827', marginBottom: 4 }}>
            {t('connect.csv_step1_title')}
          </h2>
          <p style={{ fontSize: 13, color: '#9CA3AF', marginBottom: 16 }}>
            {t('connect.csv_step1_sub')}
          </p>
            ...
            {TEMPLATES.map(tmpl => (
              <div
                ...
                style={{
                  ...
                  border: `2px solid ${selectedTemplate === tmpl.id ? tmpl.color : '#E5E7EB'}`,
              ...
              >
                 ...
                 <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  ...
                  <button
                    ...
                    style={{ padding: '8px 16px', background: tmpl.bg, color: tmpl.color, border: `1px solid ${tmpl.color}40`, borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer', flexShrink: 0, whiteSpace: 'nowrap' }}
                  >
                    {t('connect.csv_template_btn')}
                  </button>
                </div>
        ...
        {selectedTemplate && (
          <div style={{ marginBottom: 32 }}>
            ...
            <label style={{ display: 'block', padding: '32px', background: '#fff', border: '2px dashed #E5E7EB', borderRadius: 14, textAlign: 'center', cursor: 'pointer', marginBottom: 16, direction: 'ltr' }}>
              ...
              {file ? (
                <div>
                  ...
                  <p style={{ fontSize: 13, color: '#6B7280', margin: 0 }}>{(file.size / 1024).toFixed(1)} KB · {t('connect.csv_click_change')}</p>
                </div>
              ) : (
                <div>
                  <p style={{ fontSize: 32, marginBottom: 8 }}>📁</p>
                  <p style={{ fontSize: 15, fontWeight: 600, color: '#374151', marginBottom: 4 }}>{t('connect.csv_click_upload')}</p>
                  <p style={{ fontSize: 13, color: '#9CA3AF', margin: 0 }}>{t('connect.csv_drag_drop')}</p>
                </div>
              )}
            </label>
            ...
            {preview.length > 0 && (
              <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E5E7EB', padding: '16px', marginBottom: 16, overflowX: 'auto' }}>
                <p style={{ fontSize: 12, color: '#6B7280', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 10 }}>
                  {t('connect.csv_preview').replace('{n}', String(preview.length))}
                </p>
            ...
            >
              {uploading ? t('connect.csv_analysing') : t('connect.csv_upload_btn')}
            </button>
          </div>
        )}
      </div>
