...
const AI_OPPORTUNITY_SIGNALS: Record<string, {
  title: string
  description: string
  saving: string
  ...
  activation_drop:         { title: 'AI Onboarding Agent',        description: 'Automate activation sequences for users who go silent after signup',          saving: '8-12 hrs/week',  complexity: 'medium' },
  nps_decline:             { title: 'AI Feedback Analysis',       description: 'Automatically analyse and categorise NPS responses to find patterns',         saving: '4-6 hrs/week',   complexity: 'low'    },
  csat_decline:            { title: 'AI Support Optimisation',    description: 'Identify root causes of low satisfaction and suggest fixes automatically',    saving: '5-8 hrs/week',   complexity: 'low'    },
  cycle_time_increase:     { title: 'AI Workflow Automation',     description: 'Identify bottlenecks in your delivery pipeline and automate handoffs',        saving: '6-10 hrs/week',  complexity: 'medium' },
  ...
  session_duration_drop:   { title: 'AI Personalisation',         description: 'Dynamically adapt content to keep users engaged longer',                      saving: 'Revenue impact', complexity: 'high'   },
  blocked_tickets_spike:   { title: 'AI Workflow Automation',     description: 'Automatically detect and resolve common blockers before they stall the team', saving: '4-6 hrs/week',   complexity: 'medium' },
}

const complexityLabel: Record<string, string> = {
  low:    '1–2 weeks · No developer required',
  medium: '3–4 weeks · Requires technical lead',
  high:   '3+ months · Dedicated engineering squad',
}

function calculateAIReadiness(signals: SignalWithFlags[], assessment: Assessment | null): {
  score: number
  opportunities: Array<{ signal_type: string; severity: string } & typeof AI_OPPORTUNITY_SIGNALS[string]>
  ...
  const opportunities = Object.entries(AI_OPPORTUNITY_SIGNALS)
    ...
    .map(([type, data]) => {
      const highestSeverity = activeSignals
        ...
        .reduce((highest, s) => {
          return (severityRank[s.severity] ?? 0) > (severityRank[highest] ?? 0) ? s.severity : highest
        }, 'watch' as string)
...
export default async function DashboardPage() {
  ...
  // Lightweight count query — status only, no full rows needed
  ...
  const scoringInputs: ScoringInput[] = activeRealSignals.map(s => ({
  ...
  const aiReadiness = calculateAIReadiness(analysedSignals, assessment)
  ...
  const showAssessmentBanner = !!assessment && hasNoSources
  const aiSectionCopy        = activeRealSignals.length > 0
    ? 'Based on your active signals, these areas are ready for AI automation'
    : 'Based on your business profile and assessment'
    ...
    <main  style={{ minHeight: '100vh', background: '#F9FAFB', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '36px 24px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 13, color: '#7C3AED', margin: 0 }}>
                  Upgrade to Navigator to unlock your Action Digest, automated weekly scans, and unlimited tool connections
                </p>
              </div>
            </div>
            {STRIPE_PAYMENT_LINK ? (
        ...
        <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
              ...
              {!aiReadiness.hasEnoughData ? (
                <>
                  ...
                  <span style={{ fontSize: 12, color: '#A78BFA', display: 'block', marginBottom: 8 }}>Need more data to calculate</span>
                  {!assessment && <a href="/assessment" style={{ fontSize: 11, color: '#7C3AED', textDecoration: 'none', background: '#EDE9FE', borderRadius: 8, padding: '5px 10px', display: 'inline-block', fontWeight: 600 }}>Take assessment →</a>}
                </>
              ) : aiReadiness.opportunities.length === 0 ? (
                <>
                  <span style={{ fontSize: 36, fontWeight: 900, color: '#DDD6FE', lineHeight: 1, display: 'block', marginBottom: 6 }}>N/A</span>
                  <span style={{ fontSize: 12, color: '#A78BFA', display: 'block' }}>No AI opportunities in current data</span>
                </>
              ) : (
                <>
                  <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, marginBottom: 6 }}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 600, color: '#7C3AED' }}>{aiReadiness.opportunities.length} opportunit{aiReadiness.opportunities.length === 1 ? 'y' : 'ies'} found</span>
                </>
              )}
            </div>
            ...
            <div id="tour-signals" style={{ background: '#fff', borderRadius: 16, border: '1px solid #E5E7EB', padding: '20px 22px' }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                ...
                {activeRealSignals.length > 0 && activeAssessmentSignals.length > 0 && (
                  <span style={{ fontSize: 11, color: '#9CA3AF' }}>· {activeRealSignals.length} tools, {activeAssessmentSignals.length} assessment</span>
                )}
                {activeRealSignals.length === 0 && activeAssessmentSignals.length > 0 && (
                  <span style={{ fontSize: 11, color: '#9CA3AF' }}>{activeAssessmentSignals.length} from assessment</span>
                )}
          ...
          {/* ── Goals section — below score cards ── */}
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 12 }}>
                {(activeGoals ?? []).map(goal => {
                  ...
                  // Direction-aware progress using start_value
                  ...
                  // Upsell eligibility — time gate (same logic as GoalsSection)
                  ...
                  return (
                    <div key={goal.id as string} style={{
                      background: '#fff', borderRadius: 14,
                      border: `1.5px solid ${isAtRisk ? '#FECACA' : '#E5E7EB'}`,
                      padding: '16px 18px',
                    }}>
                      {/* Header */}
                      ...
                      <div style={{ height: 6, background: '#F3F4F6', borderRadius: 99, marginBottom: 6 }}>
                        <div style={{
                          ...
                          background: isAtRisk ? '#DC2626' : '#2563EB',
                          width: `${progressPct}%`,
                      ...
                      {/* Upsell — time-gated, severity-gated, tier-aware */}
                      {upsellShow && (
                        <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid #FECACA' }}>
                          ...
                          {isNavigator ? (
                            <a
                              href={`${meta.serviceUrl}&goal=${goal.signal_type}&current=${goal.current_value ?? ''}&target=${goal.target_value}&unit=${meta.unit}`}
      ...
      Reconnect →
                ...
                {conflictedSignals.map(signal => {
                  ...
                  return (
                    <div key={signal.id} style={{ background: '#fff', borderRadius: 10, padding: '12px 16px', border: '1px solid #FDE68A', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                      ...
                      <a href="/signals" style={{ fontSize: 12, color: '#D97706', fontWeight: 600, textDecoration: 'none', whiteSpace: 'nowrap', flexShrink: 0 }}>Resolve →</a>
                    </div>
                  )
                })}
              </div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: 12, borderTopWidth: 1, borderTopStyle: 'solid', borderTopColor: '#FDE68A', flexWrap: 'wrap', gap: 8 }}>
                <p style={{ fontSize: 12, color: '#92400E', margin: 0 }}>
                  ⓘ Unresolved conflicts default to tool source in your next Action Digest
                </p>
          ...
          {top3Signals.length > 0 ? (
                    ...
                    return (
                      <div key={signal.id} style={{ padding: '14px 16px', background: severityBg(signal.severity), borderRadius: 12, border: `1px solid ${severityBorder(signal.severity, !!conflictFlag)}` }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
                          ...
                          {confirmFlag && (
                            <span style={{ fontSize: 10, color: '#059669', background: '#ECFDF5', padding: '2px 7px', borderRadius: 20, fontWeight: 600 }}>
                              ✓ Confirmed by {sourceLabel[confirmFlag.bySource] ?? confirmFlag.bySource}
                            </span>
                          )}
                          {conflictFlag && (
                            <span style={{ fontSize: 10, color: '#D97706', background: '#FFFBEB', padding: '2px 7px', borderRadius: 20, fontWeight: 600 }}>
                              ⚠ Conflicts with {sourceLabel[conflictFlag.bySource] ?? conflictFlag.bySource}
                            </span>
                          )}
                        </div>
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
                  ...
                  {criticalCount > 0 && (
                    <a href="/signals?filter=critical" style={{ padding: '9px 20px', background: '#FEF2F2', color: '#DC2626', borderRadius: 9, fontSize: 13, fontWeight: 600, textDecoration: 'none' }}>{criticalCount} critical</a>
                  )}
                </div>
              </div>
            ) : (
    ...
    <a href="/connect" style={{ padding: '12px 24px', background: '#F9FAFB', color: '#374151', border: '1px solid #E5E7EB', borderRadius: 10, fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>
      Connect a tool →
              ...
              {sources && sources.length > 0 ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {sources.map((s, index) => {
                    ...
                    const isReady     = nextScan === 'Ready now' || nextScan === 'Ready to scan'
                      ...
                      <div key={`${s.source_type}-${csvLabel}`} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          ...
                          <div>
                            ...
                            <p style={{ fontSize: 11, color: isExpired ? '#D97706' : isReady ? '#059669' : '#9CA3AF', margin: 0 }}>{isExpired ? 'Needs reconnection' : `Next scan: ${nextScan}`}</p>
                          </div>
                        </div>
                        {isExpired ? (
          ...
          <div id="tour-impact" style={{ background: '#fff', borderRadius: 16, border: '1px solid #E5E7EB', padding: '24px 28px', marginBottom: 20 }}>
            ...
            {hasCycleData && (
              <div style={{ display: 'flex', flexDirection: 'row', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
                {cycleWorsening > 0 && (
                  <div style={{ padding: '8px 14px', background: '#FEF2F2', borderRadius: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ fontSize: 14 }}>↓</span>
                    <span style={{ fontSize: 12, fontWeight: 600, color: '#DC2626' }}>{cycleWorsening} worsening</span>
                  </div>
                )}
                {cycleImproving > 0 && (
                  <div style={{ padding: '8px 14px', background: '#ECFDF5', borderRadius: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ fontSize: 14 }}>↑</span>
                    <span style={{ fontSize: 12, fontWeight: 600, color: '#059669' }}>{cycleImproving} improving</span>
                  </div>
                )}
                {cycleUnchanged > 0 && (
                  <div style={{ padding: '8px 14px', background: '#FFFBEB', borderRadius: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ fontSize: 14 }}>→</span>
                    <span style={{ fontSize: 12, fontWeight: 600, color: '#D97706' }}>{cycleUnchanged} unchanged</span>
                  </div>
                )}
              </div>
            )}
            {trendSignals && trendSignals.length > 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {([...(trendSignals as Array<Record<string, unknown>>)]
                  ...
                  .sort((a, b) => {
                    const priority = (s: Record<string, unknown>) =>
                      s.trend === 'worsening' ? 3 : (s.trend === 'unchanged' && s.severity === 'critical') ? 2 : 1
                    ...
                    const trendConfig  = isPersisting
                      ...
                      : ({
                        ...
                        } as Record<string, { icon: string; label: string; color: string; bg: string }>)[signal.trend as string] ?? { icon: '·', label: 'Tracking', color: '#6B7280', bg: '#F9FAFB' }
                    return (
                      <div key={signal.id as string} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', background: trendConfig.bg, borderRadius: 10 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <p style={{ fontSize: 11, color: '#6B7280', margin: 0 }}>
                            Was {String(signal.previous_value ?? '—')} → Now {String(signal.value ?? '—')} · {sourceLabel[signal.source as string] ?? String(signal.source)}
                          </p>
                        </div>
                        <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 20, background: '#fff', color: trendConfig.color, fontWeight: 700, border: `1px solid ${trendConfig.color}30`, flexShrink: 0 }}>
                          {trendConfig.label}
                        </span>
                      </div>
                    )
                  })}
                <a href="/measure" style={{ fontSize: 13, color: '#2563EB', textDecoration: 'none', fontWeight: 600, display: 'block', marginTop: 8 }}>View full measure →</a>
              </div>
            ) : (
          ...
          {/* ── Action Digest — Navigator only ── */}
          {!isFreeTier && (
            <div style={{ background: latestDigest ? '#F5F3FF' : '#F9FAFB', borderRadius: 16, border: latestDigest ? '1px solid #DDD6FE' : '1px solid #E5E7EB', padding: '24px 28px', marginBottom: 20 }}>
              ...
              {latestDigest && (
                <p style={{ fontSize: 14, color: '#374151', lineHeight: 1.7, margin: 0, paddingTop: 14, borderTopWidth: 1, borderTopStyle: 'solid', borderTopColor: '#EDE9FE' }}>
          ...
          <div id="tour-ai-opps" style={{ background: '#fff', borderRadius: 16, border: '1px solid #DDD6FE', padding: '24px 28px', marginBottom: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: showAssessmentBanner || aiReadiness.opportunities.length > 0 ? 12 : 16 }}>
              ...
              {aiReadiness.opportunities.length > 0 && (
                <div style={{ background: '#F5F3FF', borderRadius: 12, padding: '8px 16px', flexShrink: 0 }}>
                  <span style={{ fontSize: 20, fontWeight: 800, color: '#7C3AED' }}>{aiReadiness.score}</span>
                  <span style={{ fontSize: 13, color: '#7C3AED' }}>/100 ready</span>
                </div>
              )}
            </div>
            {showAssessmentBanner && (
              <div style={{ background: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: 12, padding: '14px 18px', marginBottom: 16, display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                <span style={{ fontSize: 18, flexShrink: 0 }}>🔌</span>
                <div>
                  <p style={{ fontSize: 14, fontWeight: 700, color: '#1D4ED8', margin: '0 0 4px' }}>Your AI profile is ready — now connect your live data</p>
                  <p style={{ fontSize: 13, color: '#3B82F6', margin: '0 0 10px', lineHeight: 1.5 }}>
                    Your assessment gives us context about your business. Connect your tools so Elvanis can detect real-time AI opportunities specific to what is happening in your operations right now.
                  </p>
                  <a href="/connect" style={{ fontSize: 13, color: '#1D4ED8', fontWeight: 600, textDecoration: 'none' }}>Connect your tools →</a>
                </div>
              </div>
            )}
            {aiReadiness.opportunities.length > 0 ? (
                   ...
                   {aiReadiness.opportunities.slice(0, 3).map(opp => (
                    <div key={opp.signal_type} style={{ background: '#F5F3FF', borderRadius: 12, padding: '16px', border: '1px solid #EDE9FE' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                        ...
                        <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 20, background: '#fff', color: complexityColor(opp.complexity), fontWeight: 700, border: `1px solid ${complexityColor(opp.complexity)}30` }}>{opp.complexity}</span>
                      </div>
                      <p style={{ fontSize: 10, color: complexityColor(opp.complexity), margin: '0 0 8px', fontWeight: 500 }}>{complexityLabel[opp.complexity]}</p>
                      <p style={{ fontSize: 12, color: '#6D28D9', marginBottom: 8, lineHeight: 1.5 }}>{opp.description}</p>
                      <p style={{ fontSize: 11, color: '#7C3AED', fontWeight: 600, margin: 0 }}>⏱ Saves {opp.saving}</p>
                      {activeRealSignals.length === 0 && (
                        <p style={{ fontSize: 10, color: '#9CA3AF', margin: '8px 0 0', fontStyle: 'italic' }}>Based on assessment — connect tools to verify</p>
                      )}
                    </div>
                  ))}
                </div>
              ...
              <div style={{ padding: '24px', background: '#F5F3FF', borderRadius: 12, textAlign: 'center' }}>
                <p style={{ fontSize: 14, fontWeight: 600, color: '#6D28D9', margin: '0 0 8px' }}>
                  {!aiReadiness.hasEnoughData ? 'Not enough data to detect AI opportunities' : 'No AI opportunities in current data'}
                </p>
                <p style={{ fontSize: 13, color: '#A78BFA', margin: '0 0 16px' }}>
                  {!aiReadiness.hasEnoughData ? 'Take the assessment or connect tools to get at least 2 signals.' : 'Your current signals do not indicate areas where AI automation would help.'}
                </p>
                <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
                  <a href="/connect" style={{ padding: '9px 20px', background: '#7C3AED', color: '#fff', borderRadius: 9, fontSize: 13, fontWeight: 600, textDecoration: 'none' }}>Connect tools →</a>
                  {!assessment && <a href="/assessment" style={{ padding: '9px 20px', background: '#fff', color: '#7C3AED', borderRadius: 9, fontSize: 13, fontWeight: 600, textDecoration: 'none', border: '1px solid #DDD6FE' }}>Take assessment</a>}
                </div>
              </div>
            )}
          </div>
