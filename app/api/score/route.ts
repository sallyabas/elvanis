...
Respond with JSON only. Do NOT include overall_score — it is calculated server-side.`
  ...
  ${JSON.stringify({
    ...
    const urgencyLevel = urgencyNorm ?? 'normal'
        ...
        // ── Validate Arabic content for leaked English words ──
        const { validateArabicField } = await import('@/lib/content-validator')

        if (diagnosis.language === 'ar') {
          if (diagnosis.overall_summary) {
            diagnosis.overall_summary = await validateArabicField({
              admin: supabase, founderId,
              fieldLabel: 'assessment.overall_summary',
              englishText: '', // primary-language content has no English source to fall back to
              arabicText: diagnosis.overall_summary as string,
            })
          }
          if (diagnosis.closing_message) {
            diagnosis.closing_message = await validateArabicField({
              admin: supabase, founderId,
              fieldLabel: 'assessment.closing_message',
              englishText: '',
              arabicText: diagnosis.closing_message as string,
            })
          }
          const primaryConstraint = diagnosis.primary_constraint as Record<string, string> | undefined
          if (primaryConstraint?.summary) {
            primaryConstraint.summary = await validateArabicField({
              admin: supabase, founderId,
              fieldLabel: 'assessment.primary_constraint.summary',
              englishText: '',
              arabicText: primaryConstraint.summary,
            })
          }
        }
    
        if (isTranslated && altLanguage === 'ar') {
          if (overallSummaryAlt) {
            overallSummaryAlt = await validateArabicField({
              admin: supabase, 
              founderId,
              fieldLabel: 'assessment.overall_summary_alt',
              englishText: diagnosis.language === 'en' ? (diagnosis.overall_summary as string) : '',
              arabicText: overallSummaryAlt,
            })
          }
          if (closingMessageAlt) {
            closingMessageAlt = await validateArabicField({
              admin: supabase, 
              founderId,
              fieldLabel: 'assessment.closing_message_alt',
              englishText: diagnosis.language === 'en' ? (diagnosis.closing_message as string) : '',
              arabicText: closingMessageAlt,
            })
          }
        }
    
        // ── Validate array-based fields for completeness (top_3_findings, priority_order) ──
        const { validateArrayCompleteness } = await import('@/lib/content-validator')
    
        if (Array.isArray(diagnosis.top_3_findings)) {
          diagnosis.top_3_findings = await validateArrayCompleteness({
            admin: supabase, founderId,
            fieldLabel: 'assessment.top_3_findings',
            items: diagnosis.top_3_findings as Array<Record<string, unknown>>,
            expectedLength: 3,
            requiredFields: ['finding', 'impact'],
          })
        }
    
        if (Array.isArray(diagnosis.priority_order)) {
          diagnosis.priority_order = await validateArrayCompleteness({
            admin: supabase, founderId,
            fieldLabel: 'assessment.priority_order',
            items: diagnosis.priority_order as Array<Record<string, unknown>>,
            expectedLength: 2,
