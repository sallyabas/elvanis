...
export type RunScanParams = {
  ...
  force?: boolean
  sendEmail?: boolean
  triggeredBy: 'manual' | 'cron' | 'connect' | 'admin'
}
...
export type RunScanResult = {
  ...
  error?: string
  status?: number
  results?: Array<{ source: string; signals: number; inserted: number; updated: number; note?: string; error?: string }>
  masterScanId?: string
}

/**
 * Core scan orchestration — loops connected sources, calls each scraper,
 * records results, recalculates health score, checks goals, optionally emails.
 *
 * INTENTIONALLY CONTAINS NO RATE-LIMITING OR TIER-GATING LOGIC.
 * Those are founder-protection concerns and belong only in app/api/scan/route.ts
 * for founder calls. Any caller of this function — founder route or admin
 * route — gets full execution with no restriction. If a future restriction
 * is needed for founders, add it to app/api/scan/route.ts only; this
 * function must stay restriction-free.
 */
export async function runScan(params: RunScanParams): Promise<RunScanResult> {
  const { founderId, sourceType, force, sendEmail = false, triggeredBy } = params
  ...
  try {
    ...
    if (!founder) return { success: false, error: 'Founder not found', status: 404 }
    ...
    if (!sources || sources.length === 0) {
      return { success: false, error: 'No connected sources found', status: 400 }
    }
    ...
    if (masterErr || !masterRow) {
      console.error('[run-scan] master row insert failed:', masterErr?.message)
      return { success: false, error: 'Failed to initialise scan', status: 500 }
    }

    masterScanId = masterRow.id
    console.log(`[run-scan] master row created: ${masterScanId} triggered_by=${triggeredBy}`)
    ...
    if (updateErr) console.error('[run-scan] master row update failed:', updateErr.message)
    ...
    if (actuallyRan) {
      ...
      if (healthScore !== -1) {
        ...
        if (healthErr) console.error('[run-scan] health_score_history insert failed:', healthErr.message)
        else console.log(`[run-scan] health score saved: ${healthScore}`)
      }
      ...
      if (currentSignals && currentSignals.length > 0) {
        ...
        try {
          ...
          console.error('[run-scan] signal_snapshots insert failed:', msg)
      ...
      if (founder.email && sendEmail) {
        try {
          ...
          if (emailError) console.error('[run-scan] email error:', emailError)
          else console.log(`[run-scan] email sent to ${founder.email}`)
        } catch (emailErr) {
          console.error('[run-scan] email send failed:', emailErr)
        }
      }
    }

    console.log(`[run-scan] complete — master=${masterScanId} status=${scanStatus} new=${totalInserted} updated=${totalUpdated}`)
    scanCompleted = true
    return { success: true, results, masterScanId }
